...
CREATED_BY="ReproRebilling"

# Fetch each invoice individually to get items
show_invoices() {
    local acct="$1"
    local label="$2"
    echo ""
...
echo "============================================================"
echo " Reproducing re-billing using CLIENT's exact catalogs"
echo " V1 effective: Jan 1 2030 (no addon availability)"
echo " V2 effective: Jan 8 2030 (adds RemoteControl addon)"
echo " Subscription created: Jan 3 (under V1)"
echo " Plan change: Jan 10 (after V2 effective)"
echo "============================================================"

# --- Step 1: Create tenant ---
API_KEY="rebill-$(date +%s)"
API_SECRET="secret"
echo -e "\n--- Step 1: Create tenant ---"
...
# --- Step 3: Upload V1 — Client's exact catalog (no addon availability) ---
echo -e "\n--- Step 3: Upload Catalog V1 (effective Jan 1, NO addon) ---"
...
ACC_RES=$(curl -s -D - -X POST "$KB_URL/1.0/kb/accounts?callCompletion=true&callTimeoutSec=15" \
  ...
  -H "X-Killbill-ApiSecret: $API_SECRET" \
  -H "X-Killbill-CreatedBy: $CREATED_BY" \
  -d '{"name":"Client Rebilling Test","currency":"USD"}')
...
# --- Step 5: Create base subscription under V1 ---
echo -e "\n--- Step 5: Create subscription on standard-daily (V1 only exists) ---"
...
BUNDLE_ID=$(curl -s "$KB_URL/1.0/kb/subscriptions/$SUB_ID" \
  -H "Accept: application/json" \
  -H "Authorization: Basic $B64_AUTH" \
  -H "X-Killbill-ApiKey: $API_KEY" \
  -H "X-Killbill-ApiSecret: $API_SECRET" \
  | grep -o '"bundleId":"[^"]*"' | cut -d'"' -f4)
echo " Bundle: $BUNDLE_ID"

show_invoices "$ACCOUNT_ID" "INVOICES AFTER CREATION (Jan 3, V1 only)"

# --- Step 6: Upload V2 (subscription already exists under V1) ---
echo -e "\n--- Step 6: Upload Catalog V2 (effective Jan 8, WITH addon) ---"
echo "   Subscription already exists under V1."
curl -s -X POST "$KB_URL/1.0/kb/catalog/xml?callCompletion=true&callTimeoutSec=15" \
  -H "Content-Type: text/xml" \
  -H "Authorization: Basic $B64_AUTH" \
  -H "X-Killbill-ApiKey: $API_KEY" \
...
# --- Step 7: Warp clock to Jan 10 (past V2 effective date) ---
...
echo -e "\n--- Step 8: Plan change to SAME plan (standard-daily) ---"
...
echo "============================================================"
echo " CHECK ABOVE:"
echo "  - How many FIXED charges? (1 = no rebill, 2 = rebill)"
echo "  - Any REPAIR_ADJ items? (proration adjustments)"
echo "  - Any CBA_ADJ items? (credit adjustments)"
echo "============================================================"